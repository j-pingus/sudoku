...
public class SudokuApplication {
	private static final Logger LOG = Logger.getLogger(SudokuApplication.class);
	...
	private JMenuItem getHighlightPairsMenuItem() {
		if (highlightPairsMenuItem == null) {
			...
			highlightPairsMenuItem.setAccelerator(KeyStroke
					.getKeyStroke(KeyEvent.VK_P, ActionEvent.ALT_MASK));
	...
	private JMenuItem getProvideHintMenuItem() {
		if (provideHintMenuItem == null) {
			...
			provideHintMenuItem.setAccelerator(KeyStroke
	...
	class HighlightAction extends AbstractAction {
		...
		public HighlightAction(int value) {
			...
			putValue(LONG_DESCRIPTION,
					"Highligt value " + value + " everywhere in the matrice");
			putValue(SHORT_DESCRIPTION, "show " + value + "'s");
			putValue(ACTION_COMMAND_KEY, "highlight_" + value);
			putValue(ACCELERATOR_KEY, KeyStroke
					.getKeyStroke(KeyEvent.VK_0 + value, ActionEvent.ALT_MASK));
		}
	...
	class JTabbedSudoku extends JPanelMatrice
			implements SudokuPresenter, CellListener {
		/**
		...
		public void removeOption(CellReference ref, int value) {
			MatriceAction action = new MatriceAction(Strategies.GUESSING)
					.removeChoices(matrice, ref, value);
		...
		public void unsetHint(int row, int col, int value) {
			new MatriceAction(Strategies.HUMAN).removeChoices(matrice,
					new CellReference(row, col), new int[] { value })
					.apply(matrice);
			showMatrice();
		}
		...
		public void saveExcel() {
			try {
				...
				Runtime.getRuntime()
						.exec("rundll32 SHELL32.DLL,ShellExec_RunDLL "
		...
		public void hint() {
			...
			try {
				...
				if (action != null) {
					...
					referencesLabel
							.setText(action.size() + " will be steps applied");
					hintDialog.pack();
