...
public class Combinatory {
    ...
    public static int[][] getTuples(int size, int... values) {
        int ret[][] = new int[(int) Combinatory.Combination(size, values.length)][size];
        getTuples(ret, 0, size, 0, values);
        return ret;
    }

    private static int getTuples(int[][] ret, int id, int size, int start, int[] values) {
        if (size == 0) {
            if (ret.length > id + 1) {
            ...
            for (int j = i + 1; j < values.length; j++) {
                for (int k = j + 1; k < values.length; k++) {
                    for (int l = k + 1; l < values.length; l++) {
                        ret[id++] = new int[]{values[i], values[j], values[k], values[l]};
                    }
                }
            }
        }
