...
public class Combinatory {
	private static final Logger LOG = Logger.getLogger(Combinatory.class);
	...
	public static void main(String[] args) {
		LOG.debug(Combination(2, 5));
		LOG.debug(CombinationWithRepetition(3, 10));
		int pairs[][] = getPairs(1, 2, 3, 4, 5);
		for (int pair[] : pairs) {
			LOG.debug(Arrays.toString(pair));
		}
	}
	...
	public static Object[][] getPairs(List values) {
		Object ret[][] = new Object[(int) Combinatory.Combination(2,
				values.size())][2];
		int id = 0;
	...
	public static int[][] getPairs(int... values) {
		int ret[][] = new int[(int) Combinatory.Combination(2,
				values.length)][2];
		int id = 0;
		for (int i = 0; i < values.length - 1; i++) {
			for (int j = i + 1; j < values.length; j++) {
				ret[id++] = new int[] { values[i], values[j] };
			}
	...
	public static int[][] getTriplets(int... values) {
		int ret[][] = new int[(int) Combinatory.Combination(3,
				values.length)][3];
		int id = 0;
		for (int i = 0; i < values.length - 1; i++) {
			for (int j = i + 1; j < values.length; j++) {
				for (int k = j + 1; k < values.length; k++) {
					ret[id++] = new int[] { values[i], values[j], values[k] };
				}
			}
		}
		return ret;
	...
	public static int[][] getTuples(int size, int... values) {
		int ret[][] = new int[(int) Combinatory.Combination(size,
				values.length)][size];
		getTuples(ret, 0, size, 0, values);
		return ret;
	}

	private static int getTuples(int[][] ret, int id, int size, int start,
			int[] values) {
		if (size == 0) {
			if (ret.length > id + 1) {
				System.arraycopy(ret[id], 0, ret[id + 1], 0, ret[id].length);
			}
			return id + 1;
		}
		for (int i = start; i <= values.length - size; i++) {
			ret[id][ret[id].length - size] = values[i];
			id = getTuples(ret, id, size - 1, i + 1, values);
		}
		return id;
	}

	public static int[][] getQuads(int... values) {
		int ret[][] = new int[(int) Combinatory.Combination(4,
				values.length)][4];
		int id = 0;
		for (int i = 0; i < values.length - 1; i++) {
			for (int j = i + 1; j < values.length; j++) {
	...
	public static int[][] getQuints(int... values) {
		int ret[][] = new int[(int) Combinatory.Combination(5,
				values.length)][5];
		int id = 0;
		for (int i = 0; i < values.length - 1; i++) {
			for (int j = i + 1; j < values.length; j++) {
	...
	public static int[][] getSextes(int... values) {
		int ret[][] = new int[(int) Combinatory.Combination(6,
				values.length)][6];
		int id = 0;
		for (int i = 0; i < values.length - 1; i++) {
			for (int j = i + 1; j < values.length; j++) {
